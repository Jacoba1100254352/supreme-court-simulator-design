...
public final class WorldGenerator {
    ...
    private static List<ReviewCase> generateDocket(WorldSpec spec, Random random) {
        ...
        for (int i = 0; i < spec.cases(); i++) {
            ...
            double rightsBurden = Values.clamp01(0.10 + profile.rightsRisk() * 0.52 + rightsTypeBoost + electionBoost * 0.5 + random.nextGaussian() * 0.16);
            double democraticMandate = Values.clamp01(
                    0.35 + profile.publicLegitimacy() * 0.35 - profile.weakMandateRate() * 0.22 - electionBoost * 0.12 + random.nextGaussian() * 0.16
            );
            double partisanSalience = Values.clamp01(0.12 + profile.partisanSkew() * 0.42 + profile.volatility() * 0.20 + electionBoost + random.nextDouble() * 0.25);
            double lawIdeology = random.nextBoolean() ? 1.0 : -1.0;
            lawIdeology = Values.clamp(lawIdeology * (0.25 + partisanSalience * 0.75), -1.0, 1.0);
            double emergencyPressure = Values.clamp01(spec.emergencyShare() * 0.55 + profile.volatility() * 0.20 + executiveBoost + electionBoost + emergencyTypeBoost + random.nextDouble() * 0.28);
            double requestedEmergencyRelief = Values.clamp01(emergencyPressure * 0.70 + executiveBoost * 0.20 + random.nextDouble() * 0.20);
            double defianceRisk = Values.clamp01(0.08 + executiveBoost + electionBoost * 0.6 + profile.partisanSkew() * 0.24 + random.nextDouble() * 0.26);
            double legislativeQuality = Values.clamp01(profile.legalQuality() + random.nextGaussian() * 0.13 - profile.weakMandateRate() * 0.12 - adminBoost * 0.10);
            double conflictPotential = Values.clamp01(
                    0.12 + rightsBurden * 0.22 + partisanSalience * 0.32 + emergencyPressure * 0.12 + profile.overridePressure() * 0.22 + executiveBoost * 0.08
            );
            ...
            double overridePressure = Values.clamp01(profile.overridePressure() * 0.42 + democraticMandate * 0.24 + publicAttention * 0.18 + partisanSalience * 0.16);
