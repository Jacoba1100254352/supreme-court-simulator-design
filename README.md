...
Use `make replication-check` before submission. It rebuilds tests, campaigns, diagnostics, validation dashboards, the strict paper checks, and the replication/submission packages. It then extracts the delivered replication ZIP, reruns the analytic workflow, and verifies byte equality for the frozen report CSVs and manuscript figure/table TeX fragments.

Use `make expert-review-packet` to prepare separate independent-reader and coordinator ZIPs, source-linked case cards, blank return forms, and generated evidence priorities. Start with `docs/expert-legal-coding-protocol.md` and `reports/review-data-quality-v1.md`. The semantic coverage audit distinguishes populated placeholders from substantive codes for both petition terms: `brief filed` is not an SG recommendation. Human legal review and methods signoff remain pending. Return files must be stored separately from generated templates under ignored private storage.
