...
def write_selected_campaign_table(averages: dict[str, dict[str, float]]) -> None:
    ...
    lines = [
        ...
        "\\Description{Table of selected campaign averages for rights protection, emergency irregularity, emergency downstream effect, process legitimacy, lower-court compliance, and the main tradeoff for each design.}",
        "\\footnotesize",
        "\\setlength{\\tabcolsep}{3pt}",
        "\\resizebox{\\textwidth}{!}{%",
        "\\begin{tabular}{>{\\raggedright\\arraybackslash}p{1.55in}rrrrr>{\\raggedright\\arraybackslash}p{2.35in}}",
        "\\toprule",
        "Scenario & Rights & Emerg. irregularity & Emerg. downstream & Process legit. & Lower ct. & Main tradeoff \\\\",
        "\\midrule",
    ]
...
def write_emergency_walkthrough_table() -> None:
    ...
    lines = [
        ...
        "\\toprule",
        "Design & Emerg. orders & Reasoned orders & Merits accel. & Irregularity & Downstream & Process legit. \\\\",
        "\\midrule",
    ]
    for key, label in selected:
        row = rows[key]
