...
def write_calibration_guardrails() -> None:
    ...
    source_labels = {
        ...
        "scotus-certiorari-docketed-cohort-ot2023": "official dockets",
        "scotus-certiorari-docketed-cohort-ot2024": "official dockets",
    }

    def source_basis(row: dict[str, str]) -> str:
        source_parts = [
            source_labels.get(key, key)
            for key in row["sourceKey"].split(";")
            if key
        ]
...
def read_benchmark_readiness_rows() -> list[dict[str, str]]:
    with BENCHMARK_READINESS_CSV.open(newline="") as handle:
        return list(csv.DictReader(handle))


def write_benchmark_readiness_table() -> None:
    rows = []
    all_rows = read_benchmark_readiness_rows()
    per_pathway: dict[str, int] = defaultdict(int)
    selected_keys = set()
    for row in all_rows:
        if row["pathway"] in per_pathway:
            continue
        rows.append(row)
        per_pathway[row["pathway"]] += 1
        selected_keys.add((row["pathway"], row["simulatorMetric"]))
    for row in all_rows:
        if len(rows) >= 8:
            break
        key = (row["pathway"], row["simulatorMetric"])
        if key in selected_keys or per_pathway[row["pathway"]] >= 2:
            continue
        rows.append(row)
        per_pathway[row["pathway"]] += 1
        selected_keys.add(key)
    rows.sort(key=lambda row: int(row["priorityRank"]))

    def short_use(value: str) -> str:
        if value.startswith("do not frame"):
            return "not validation yet"
        if value.startswith("keep as loose"):
            return "loose calibration only"
        if value.startswith("keep as design"):
            return "design context only"
        if value.startswith("can support"):
            return "bounded guardrail"
        return value
    ...
    for row in rows:
        lines.append(
            ...
            f"{tex_escape(row['pathway'].replace('_', ' '))} & "
            f"{tex_escape(row['construct'])} & "
            f"{tex_escape(row['benchmarkStatus'])} & "
            f"{tex_escape(row['requiredEvidence'])} & "
            f"{tex_escape(short_use(row['manuscriptUseAfterCompletion']))} \\\\"
        )
    lines.extend([
        "\\bottomrule",
        "\\end{tabular}",
    ...
    (TABLE_DIR / "benchmark_readiness.tex").write_text("\n".join(lines))


def write_certiorari_multi_term_table() -> None:
    ...
    labels = [
        ("paidPetitionShare", "Paid share of paid/IFP intake"),
        ("cfrRate_paid", "Court-requested response, paid"),
        ("cfrRate_ifp", "Court-requested response, IFP"),
        ("cvsgRequestRate", "CVSG request"),
        ("certStageAmicusPresenceRate", "Cert-stage amicus presence"),
        ("relistRate", "Relist presence"),
        (
            "certiorariGrantRate_docketedCohort",
            "Grant or GVR among resolved certiorari petitions",
        ),
    ]
    missing = [metric for metric, _label in labels if metric not in by_metric]
    if missing:
        raise SystemExit(
            "Multi-term certiorari table is missing metrics: "
            + ", ".join(missing)
        )
    first_term = by_metric[labels[0][0]]["firstTerm"]
    latest_term = by_metric[labels[0][0]]["latestTerm"]
    lines = [
        "% Auto-generated by paper/scripts/generate_figures.py",
        "\\begin{table}[hbt!]",
        "\\centering",
        (
            "\\caption{Official-docket certiorari-stage benchmarks across "
            f"{tex_escape(first_term)} and {tex_escape(latest_term)}}}"
        ),
        "\\label{tab:certiorari-multi-term}",
        (
            "\\Description{Table comparing paid intake share, "
            "court-requested response, CVSG, amicus, relist, and grant or "
            "GVR rates in two official Supreme Court docketed-intake cohorts.}"
        ),
        "\\small",
        "\\setlength{\\tabcolsep}{4pt}",
        ...
        "\\toprule",
        (
            f"Measure & {tex_escape(first_term)} & "
            f"{tex_escape(latest_term)} & $\\Delta$ pp & Pooled \\\\"
        ),
        "\\midrule",
    ]
    for metric, label in labels:
        ...
        lines.append(
            f"{tex_escape(label)} & {first:.2f}\\% & {latest:.2f}\\% & "
    ...
    lines.extend(
        [
            ...
            "\\end{tabular}",
            (
                "\\par\\footnotesize Each term is an independently enumerated "
                "paid/IFP published-statistics snapshot cohort. Pooled rates "
                "sum term numerators and denominators. The comparison is "
                "descriptive, excludes undocketed submissions, and is not "
                "limited to constitutional-review petitions. The grant/GVR "
                "row excludes three OT2024 petitions still pending or held "
                "as of the snapshot. The OT2024 "
                "manifest separately discloses same-cutoff-date dockets above "
                "the Journal count-defined ranges."
            ),
            "\\end{table}",
            "",
        ]
    )
    (TABLE_DIR / "certiorari_multi_term.tex").write_text("\n".join(lines))
...
def main() -> None:
    ...
    write_benchmark_readiness_table()
    write_certiorari_multi_term_table()
