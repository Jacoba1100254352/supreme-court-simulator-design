...
Read `technical-supplement.pdf` alongside the main manuscript. Its Tables S1-S4 preserve alternative priority scores, full pipeline diagnostics, mechanical-versus-downstream interpretation rules, and representative model weights. `make paper` builds both anonymous PDFs and checks both logs and input freshness. The word-limit gate counts the full rendered main PDF using `pdftotext -layout`, including tables, captions, numbers, declarations and references; the source-only estimate printed before compilation does not establish compliance. The supplement is delivered separately, with its upload category subject to the journal's requirements.
