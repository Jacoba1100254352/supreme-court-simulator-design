...
The review set now includes `paper/technical-supplement.pdf`. It preserves detailed diagnostic tables and model weights as Tables S1-S4; no analytic values were removed. `make paper` and `make paper-strict-check` enforce the main article's 10,000-word ceiling using all whitespace-delimited text from the rendered PDF, including tables and references. The pre-build source-prose estimate is explicitly non-authoritative. Both PDFs are rebuilt, checked for unresolved references and overflow, and included in the tested full and anonymous packages. Confirm the journal's supplementary-material upload treatment before submission.
...
- The paper inventory contains one primary manuscript with an anonymous technical supplement. `paper/abstract-variants.md` and `paper/title-page.tex` are other submission companions, not separate subpapers; no standalone breakoff manuscript currently requires an independent publication gate.
...
- `make paper-check` verifies anonymous review mode, JLC template options, hidden hyperlink borders, author-date fallback, declarations, figure/table descriptions and references, data availability placement, required main/supplement tables, source-audit coverage, source provenance, a mechanics appendix, and report consistency. `make paper` adds the rendered-PDF word ceiling after compilation; the pre-build source estimate cannot prove it.
